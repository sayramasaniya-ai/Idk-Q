...
local h, hrp
local setInfJump, setAntiRag, setFps, setMedusaCounter, setAutoTpDown, setUnwalkToggle, setBatAimbot, setAimbot, setAutoUnmob
local setupMedusaCounter, stopMedusaCounter, startAntiRagdoll, stopAntiRagdoll
local startBatAimbot, stopBatAimbot, startAimbot, stopAimbot
local applyFPSBoost
...
local function autoSaveConfig()
        ...
        task.delay(0.5, function()
                local cfg = {
                        ...
                        aimbot           = State.aimbotEnabled,
                        autoUnmob        = State.autoUnmobEnabled,
                }
...
startAimbot = function()
        ...
        aimbotConn = RunService.Heartbeat:Connect(function()
                ...
                if target and target.Character then
                        local tHrp = target.Character:FindFirstChild("HumanoidRootPart")
                        if tHrp then
                                -- Rotate HRP to face target (no anchoring, can still move)
                                local lookAt = Vector3.new(tHrp.Position.X, r.Position.Y, tHrp.Position.Z)
                                r.CFrame = CFrame.new(r.Position, lookAt)
                                -- Lock camera onto target
                                local cam = workspace.CurrentCamera
                                if cam then
                                        local offset = Vector3.new(0, 2, 0)
                                        cam.CFrame = CFrame.new(r.Position + Vector3.new(0, 3, 0), tHrp.Position + offset)
                                end
                        end
                end
        end)
...
-- ================= GUI CLEANUP =================
for _, name in pairs({"JispiHubGUI", "DarkAngelMiniGUI"}) do
...
local function saveMiniPosition()
        if miniBtn then
                ...
                local str = string.format("%.3f,%.1f,%.3f,%.1f", pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset)
                pcall(function() writefile("DarkAngelMiniBtnPos.txt", str) end)
        end
end

local function loadMiniPosition()
        local savedPos = nil
        pcall(function() savedPos = readfile("DarkAngelMiniBtnPos.txt") end)
...
miniBtn.BorderSizePixel = 0; miniBtn.Text = "DARK\nANGEL"
...
setAimbot = makeToggleRow("Aimbot [G]", nil, false, function(on)
...
local function szgStopMoving()
        ...
        if humP then
                humP:Move(Vector3.zero, false)
                humP.WalkSpeed = 16  -- restore default walk speed
        end
        if szgProxy then
                pcall(function() szgProxy:Destroy() end)
                szgProxy = nil
        end
end
        ...
        szgConn = RunService.Stepped:Connect(function()
                ...
                if dist < 2.5 then
                        ...
                        if szgWpIndex > #szgWaypoints then
                                ...
                                szgStopMoving()
                                -- Auto Unmob: restore walk speed and unanchor after patrol
                                if State.autoUnmobEnabled then
                                        ...
                                        if r2 then r2.Anchored = false end
                                end
                                return
                        end
...
-- ===== SLIDER HELPER (styled to match Dark Angel Mini) =====
...
setAutoUnmob = makeToggleRow("Auto Unmob After Play", nil, false, function(on)
        State.autoUnmobEnabled = on
        autoSaveConfig()
end)

makeGap(6)

-- ================= MOBILE AUTO PLAY BUTTONS =================
local mobileAutoLeft = Instance.new("TextButton", gui)
mobileAutoLeft.Name = "MobileAutoLeft"
mobileAutoLeft.Size = UDim2.new(0,64,0,64)
mobileAutoLeft.Position = UDim2.new(1,-150,1,-90)
mobileAutoLeft.BackgroundColor3 = Color3.fromRGB(10,10,10)
mobileAutoLeft.BorderSizePixel = 0
mobileAutoLeft.Text = "AUTO\nLEFT"
mobileAutoLeft.TextColor3 = C_DIM
mobileAutoLeft.Font = Enum.Font.GothamBlack
mobileAutoLeft.TextSize = 10
mobileAutoLeft.AutoButtonColor = false
mobileAutoLeft.ZIndex = 50
mobileAutoLeft.Active = true
Instance.new("UICorner", mobileAutoLeft).CornerRadius = UDim.new(0,12)
local malStroke = Instance.new("UIStroke", mobileAutoLeft); malStroke.Color = C_BORDER2; malStroke.Thickness = 1.5

local mobileAutoRight = Instance.new("TextButton", gui)
mobileAutoRight.Name = "MobileAutoRight"
mobileAutoRight.Size = UDim2.new(0,64,0,64)
mobileAutoRight.Position = UDim2.new(1,-80,1,-90)
mobileAutoRight.BackgroundColor3 = Color3.fromRGB(10,10,10)
mobileAutoRight.BorderSizePixel = 0
mobileAutoRight.Text = "AUTO\nRIGHT"
mobileAutoRight.TextColor3 = C_DIM
mobileAutoRight.Font = Enum.Font.GothamBlack
mobileAutoRight.TextSize = 10
mobileAutoRight.AutoButtonColor = false
mobileAutoRight.ZIndex = 50
mobileAutoRight.Active = true
Instance.new("UICorner", mobileAutoRight).CornerRadius = UDim.new(0,12)
local marStroke = Instance.new("UIStroke", mobileAutoRight); marStroke.Color = C_BORDER2; marStroke.Thickness = 1.5

-- Sync mobile button visuals with SZG state
local function updateMobileBtns()
        TweenService:Create(mobileAutoLeft, TweenInfo.new(0.15), {
                BackgroundColor3 = SZG.AutoLeft and Color3.fromRGB(40,40,80) or Color3.fromRGB(10,10,10),
                TextColor3 = SZG.AutoLeft and C_WHITE or C_DIM,
        }):Play()
        TweenService:Create(malStroke, TweenInfo.new(0.15), {
                Color = SZG.AutoLeft and Color3.fromRGB(100,100,200) or C_BORDER2,
        }):Play()
        TweenService:Create(mobileAutoRight, TweenInfo.new(0.15), {
                BackgroundColor3 = SZG.AutoRight and Color3.fromRGB(40,40,80) or Color3.fromRGB(10,10,10),
                TextColor3 = SZG.AutoRight and C_WHITE or C_DIM,
        }):Play()
        TweenService:Create(marStroke, TweenInfo.new(0.15), {
                Color = SZG.AutoRight and Color3.fromRGB(100,100,200) or C_BORDER2,
        }):Play()
end

-- Patch szgUpdateBtns to also update mobile buttons
local _origUpdateBtns = szgUpdateBtns
szgUpdateBtns = function()
        _origUpdateBtns()
        updateMobileBtns()
end

mobileAutoLeft.MouseButton1Click:Connect(function()
        if SZG.AutoLeft then
                szgStopPatrol(); SZG.AutoLeft = false; szgUpdateBtns()
        else
                szgStopPatrol(); SZG.AutoRight = false; SZG.AutoLeft = true
...
mobileAutoRight.MouseButton1Click:Connect(function()
        if SZG.AutoRight then
                szgStopPatrol(); SZG.AutoRight = false; szgUpdateBtns()
        else
                szgStopPatrol(); SZG.AutoLeft = false; SZG.AutoRight = true
...
-- Make mobile auto buttons draggable
local function makeMobileAutoDraggable(btn)
        local dragging, dragInput, dragStart, startPos = false, nil, nil, nil
...
makeMobileAutoDraggable(mobileAutoLeft)
makeMobileAutoDraggable(mobileAutoRight)

updateMobileBtns()

-- ================= MOBILE AIMBOT BUTTON =================
local mobileAimbot = Instance.new("TextButton", gui)
mobileAimbot.Name = "MobileAimbot"
mobileAimbot.Size = UDim2.new(0,64,0,64)
mobileAimbot.Position = UDim2.new(1,-220,1,-90)
mobileAimbot.BackgroundColor3 = Color3.fromRGB(10,10,10)
mobileAimbot.BorderSizePixel = 0
mobileAimbot.Text = "AIM\nBOT"
mobileAimbot.TextColor3 = C_DIM
mobileAimbot.Font = Enum.Font.GothamBlack
mobileAimbot.TextSize = 11
mobileAimbot.AutoButtonColor = false
mobileAimbot.ZIndex = 50
mobileAimbot.Active = true
Instance.new("UICorner", mobileAimbot).CornerRadius = UDim.new(0,12)
local mabStroke = Instance.new("UIStroke", mobileAimbot); mabStroke.Color = C_BORDER2; mabStroke.Thickness = 1.5

local function updateMobileAimbot()
        TweenService:Create(mobileAimbot, TweenInfo.new(0.15), {
                BackgroundColor3 = State.aimbotEnabled and Color3.fromRGB(50,20,60) or Color3.fromRGB(10,10,10),
                TextColor3 = State.aimbotEnabled and C_WHITE or C_DIM,
        }):Play()
        TweenService:Create(mabStroke, TweenInfo.new(0.15), {
                Color = State.aimbotEnabled and Color3.fromRGB(160,80,200) or C_BORDER2,
        }):Play()
end

mobileAimbot.MouseButton1Click:Connect(function()
        State.aimbotEnabled = not State.aimbotEnabled
        setAimbot(State.aimbotEnabled)
        if State.aimbotEnabled then startAimbot() else stopAimbot() end
        updateMobileAimbot()
        autoSaveConfig()
end)

makeMobileAutoDraggable(mobileAimbot)
updateMobileAimbot()
...
makeKeybindRow("G",   "Toggle Aimbot")
...
footerLbl.Text = "dark angel mini  ·  v2.0"; footerLbl.TextColor3 = Color3.fromRGB(50,50,50)
...
local function loadConfig()
        ...
        if type(cfg.aimbot) == "boolean" and cfg.aimbot then
                State.aimbotEnabled = true; setAimbot(true); startAimbot()
        end
        if type(cfg.autoUnmob) == "boolean" and cfg.autoUnmob then
                State.autoUnmobEnabled = true; setAutoUnmob(true)
        end
end
...
print("✅ Dark Angel Mini v2.0 + SZG Auto Play")
